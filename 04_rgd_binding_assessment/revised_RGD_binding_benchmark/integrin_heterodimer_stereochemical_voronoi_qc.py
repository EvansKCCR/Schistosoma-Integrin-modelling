...
TWO_LETTER_ELEMENTS = {
    "AL", "AG", "AU", "BA", "BR", "CA", "CD", "CL", "CO", "CS", "CU",
    "FE", "HG", "K", "LI", "MG", "MN", "NA", "NI", "PB", "RB", "SE",
    "SI", "SN", "SR", "ZN",
}
METAL_RESNAMES = {
    "AL", "AG", "AU", "BA", "CA", "CD", "CO", "CS", "CU", "FE", "HG",
...
def clean_pdb_file(
    ...
    def infer_element(line: str) -> str:
        ...
        current = line[76:78].strip().upper() if len(line) >= 78 else ""

        # Metal ions often arrive as HETATM records with atom/residue name CA,
        # MG, etc. Phenix rejects these when the element column has drifted to
        # a single trailing letter such as "A".
        if resname in METAL_RESNAMES and atom_name in (resname, f"{resname}1", f"{resname}2"):
            return resname

        name = re.sub(r"^\d+", "", atom_name)
        if name == "CA" and resname in STANDARD_AA_NAMES:
            return "C"
        if len(name) >= 2 and name[:2] in TWO_LETTER_ELEMENTS and resname not in STANDARD_AA_NAMES:
            return name[:2]
        if current in TWO_LETTER_ELEMENTS or (len(current) == 1 and current.isalpha()):
            if not (current == "A" and resname == "CA"):
                return current
        for ch in name:
            if ch.isalpha():
                return ch
    ...
    def normalize_atom_line(line: str) -> str:
        ...
        if len(body) < 80:
            body = body.ljust(80)
        body = body[:76] + elem.rjust(2)[:2] + body[78:]
        return body + "\n"
